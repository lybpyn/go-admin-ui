    ...
    <template #wrapper>
      <el-card class="box-card">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              v-permisaction="['admin:ordGiftcardDiscounts:add']"
              type="primary"
              icon="el-icon-plus"
              size="mini"
              @click="handleAdd"
          ...
          <el-col :span="1.5">
            <el-button
              v-permisaction="['admin:ordGiftcardDiscounts:edit']"
              type="success"
              icon="el-icon-edit"
              size="mini"
              :disabled="single"
          ...
          <el-col :span="1.5">
            <el-button
              v-permisaction="['admin:ordGiftcardDiscounts:remove']"
              type="danger"
              icon="el-icon-delete"
              size="mini"
              :disabled="multiple"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listOrdGiftcardDiscounts(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleUpdate(row) {
      ...
      const id =
                row.id || this.ids
      getOrdGiftcardDiscounts(id).then(response => {
        this.form = response.data
        this.open = true
        this.title = '修改礼品卡不同类型折扣'
        this.isEdit = true
      })
    },
    /** 提交按钮 */
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id !== undefined) {
            updateOrdGiftcardDiscounts(this.form).then(response => {
            ...
            addOrdGiftcardDiscounts(this.form).then(response => {
              if (response.code === 200) {
                this.msgSuccess(response.msg)
                this.open = false
                this.getList()
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除编号为"' + Ids + '"的数据项?', '警告', {
        ...
        return delOrdGiftcardDiscounts({ 'ids': Ids })
      }).then((response) => {
        if (response.code === 200) {
          this.msgSuccess(response.msg)
          this.open = false
